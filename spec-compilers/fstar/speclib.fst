...
val uints_from_bytes_le: #a:inttype -> s:seq uint8_t{length s % (numbytes a) = 0} -> r:seq (uint_t a){length r = (length s) / (numbytes a)}
let uints_from_bytes_le #a s =
  to_seq (uints_from_bytes_le #a #((length s) / (numbytes a)) (to_lseq s))

val uints_to_bytes_le: #a:inttype -> s:seq (uint_t a){FStar.Mul.op_Star (length s) (numbytes a) <= max_size_t} -> r:seq uint8_t{length r = FStar.Mul.op_Star (length s) (numbytes a)}
let uints_to_bytes_le #a s =
  to_seq (uints_to_bytes_le #a #(length s) (to_lseq s))
